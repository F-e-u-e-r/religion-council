...
Covers the generalized judge-template validator (human OR model), committed filled model-panel passes,
...
GPT = ROOT / "docs" / "benchmarks" / "judgments" / "panel" / "retrieval-v1-model-judge-gpt.json"
...
class ModelPanelValidatorTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.gpt = json.loads(GPT.read_text("utf-8"))
    ...
    def test_model_judge_type_is_accepted(self):
        ...
        self.assertEqual(self.v.validate(self.gpt, self.fixture_keys, filled=True), [])
    ...
    def test_gpt_panel_is_blind_and_disclosed(self):
        judge = self.gpt["judge"]
        self.assertEqual(judge["judge_type"], "model")
        for field in ("id", "provider", "model_family", "blind_to", "labeling_note"):
            self.assertTrue(judge.get(field), field)
        self.assertEqual(judge["provider"], "openai")
        self.assertEqual(len(self.gpt["pool"]), 110)
        for item in self.gpt["pool"]:
    ...
    def test_cli_filled_returns_zero(self):
        ...
        self.assertEqual(self.v.main([str(GPT), "--filled"]), 0)
...
class PanelAgreementTest(unittest.TestCase):
    ...
    def test_panel_with_gpt_matches_committed_pairwise_kappas(self):
        pairs = {
            tuple(pair["judges"]): pair
            for pair in self.p.pairwise(self.p.build_raters([str(CLAUDE), str(GPT)]))
        }
        self.assertEqual(len(pairs), 3)
        self.assertEqual(pairs[("curator-1", "model-judge-claude-opus")]["kappa"], 0.4436)
        self.assertEqual(pairs[("curator-1", "model-judge-gpt")]["kappa"], 0.3863)
        self.assertEqual(pairs[("model-judge-claude-opus", "model-judge-gpt")]["kappa"], 0.8998)
