...
- Second safety axis, weaponization-first (`policies/weaponization-routing.v1.json`): if a request's evident purpose is to use the council to produce targeted attack, dehumanization, harassment, or incitement material against a religious or belief group or individual, do not start the council — attack ideas, not people or communities; offer to examine the underlying doctrine at the level of propositions instead. A `debate_start` carrying `weaponization_classification="weaponization-first"` is refused by the controller before any run work. This does not block critical, academic, historical, or comparative discussion; the guarantee is routing-only, never detection.
