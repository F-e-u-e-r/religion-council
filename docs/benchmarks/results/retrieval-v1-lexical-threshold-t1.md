# Retrieval Benchmark v1 — Candidate: lexical-threshold (threshold=1)

## Status

**Experiment only.** This report evaluates a post-retrieval lexical confidence threshold (top\_score < 1 → no support) against the v0.12.2 lexical baseline. **No default behavior changed.** **No backend selected.**

## Retriever

- retriever_kind: `project-file`
- contract_version: `religion-council/retrieval/v1`
- corpus: C0 curated references — 56 records across 8 traditions (VERSION v0.13.0)
...
- total: 0.0211s · avg query: 1.1747 ms · max query: 1.7929 ms
...
| q014 | no_answer | — | — | false_support | 布哈里聖訓實錄·開卷傳述(附傳述鏈) (score 1) |
| q015 | no_answer | — | — | false_support | 薄伽梵歌·2:20 (score 1) |
...
- `q014`: '區塊鏈與加密貨幣的投資策略' surfaced 布哈里聖訓實錄·開卷傳述(附傳述鏈) at score 1 — a spurious lexical overlap, not support.
- `q015`: '最新智慧型手機的硬體規格比較' surfaced 薄伽梵歌·2:20 at score 1 — a spurious lexical overlap, not support.
